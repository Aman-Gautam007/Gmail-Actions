...
DEFAULT_CONFIG_DIR = Path.home() / "Library" / "Application Support" / "AdvertisementCleaner"


def reminder_is_completed(title: str, list_name: str | None) -> bool:
    script = r'''
on run argv
        ...
        repeat with reminderList in lists
            if targetList is "" or name of reminderList is targetList then
                ...
                repeat with oneReminder in matches
                    if completed of oneReminder is true then return "completed"
                end repeat
            end if
        end repeat
    end tell
...
def main() -> int:
    args = parse_args()
    if not reminder_is_completed(args.reminder, args.reminder_list):
        print(f'Reminder is not completed: "{args.reminder}". Nothing changed.')
        return 0
    ...
    if not args.execute:
        print("Preview only. Nothing changed. Add --execute with the confirmation phrase to proceed.")
        return 0
    if args.confirmation != "MOVE-ADVERTISEMENT-TO-TRASH":
        print("Confirmation phrase missing or incorrect. Nothing changed.", file=sys.stderr)
        return 2
    if not message_ids:
        return 0
